...
internal class CartesianTest {
    private companion object {
        ...
        val CartesianProduct.cpArity: Int? get() = firstOrNull()?.tupleArity.takeIf { all { tuple -> tuple.tupleArity == it } }
    ...
    fun `singleton and 1-D Cartesian Product`(){
        ...
        intFamily5.asCartesianProduct.let {
            assertEquals(5, it.cpSize)
            assertEquals(1, it.cpArity)
        }
    }

    @Test
    fun `0-D Cartesian Product`() {
        val cpOfZeroFamilies: CartesianProduct = emptySequence<Family<*>>().toCartesianProduct()
        assertEquals(1, cpOfZeroFamilies.cpSize)
        assertEquals(0, cpOfZeroFamilies.cpArity)

        val emptyFamily: Family<Nothing> = emptySequence()
        val cpOf1EmptyFamily: CartesianProduct = sequenceOf(emptyFamily).toCartesianProduct()
        assertEquals(0, cpOf1EmptyFamily.cpSize)
        assertNull(cpOf1EmptyFamily.cpArity)
    }
